...
function initLocationAndLoad() {
    ...
    showMedicalMap(DEFAULT_LAT, DEFAULT_LON);
...
initLocationAndLoad();
let medicalMapInitialized = false;

function showMedicalMap(lat, lon) {

  // Prevent duplicate Leaflet initialization
  if (medicalMapInitialized) return;
  medicalMapInitialized = true;

  const map = L.map("medicalMap").setView([lat, lon], 12);
  ...
  const query = `
    [out:json];
    (
      node["amenity"="college"]["name"~"Medical", i](around:12000,${lat},${lon});
      way["amenity"="college"]["name"~"Medical", i](around:12000,${lat},${lon});
    );
    out center;
  `;
  ...
  .then(data => {
    data.elements.forEach(place => {
      ...
      const name = place.tags?.name || "Medical College";
...
function validateLocation(position) {
  ...
  // Rough check: India latitude range ≈ 6° to 37°
  if (lat < 6 || lat > 37) {
    console.warn("Invalid location detected. Using Palakkad fallback.");

    document.getElementById("safeLocations").innerText =
      "Using fallback location: Palakkad, Kerala";

    loadMedicalColleges({
      coords: {
        latitude: PALAKKAD_LAT,
        longitude: PALAKKAD_LON
      }
    });
  } else {
    loadMedicalColleges(position);
  }
}


function loadMedicalColleges(position) {
  const lat = position.coords.latitude;
  const lon = position.coords.longitude;
  ...
  L.marker([lat, lon])
    .addTo(map)
    .bindPopup("📍 Location Used for Search")
    .openPopup();

  const query = `
    [out:json];
    (
      node["amenity"="college"]["name"~"Medical"](around:12000,${lat},${lon});
      way["amenity"="college"]["name"~"Medical"](around:12000,${lat},${lon});
    );
    out center;
  `;
  ...
  fetch("https://overpass-api.de/api/interpreter", {
    ...
    .then((data) => {
      if (data.elements.length === 0) {
        document.getElementById("safeLocations").innerText =
          "No medical colleges found nearby.";
        return;
      }

      document.getElementById("safeLocations").innerText =
        "Medical colleges shown on map.";
...
function geoError() {
  document.getElementById("safeLocations").innerText =
    "Location permission denied. Using Palakkad, Kerala.";
